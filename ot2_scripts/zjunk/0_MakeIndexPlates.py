...
def run(protocol: protocol_api.ProtocolContext):
	...
	tips = protocol.load_labware('opentrons_96_filtertiprack_200ul', '8') # 20ul filter tips on deck position 1
	...
	if loadforward:
		...
		for load in csv.DictReader(loadings_parsed):
			...
			for targ in target:
				...
				target_plate=targ[0]
				target_well=targ[1]
				left_pipette.aspirate(volume, eval(source_plate)[source_well])
